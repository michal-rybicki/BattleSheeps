...
public class Game
{
	...
	//0 - pole nieprzeszukane
	//1 - uratowana owca
	//2 - zjedzona owca
	//3 - pole przeszukane przez gracza
	//4 - pole przeszukane przez wilka
	int savedSheeps[][];

	//POLA PRYWATNE---------------------------------------------------------------------------------
	private int numer_stada;
	protected int s;// faktyczny rozmiar planszy, plansza do gry dla usera ma rozmiar s-2
	private int s1;
	...
	public int liczba1, liczba2, stara_liczba1, stara_liczba2;//do algorytmu wilka
	private int l_owiec; /// liczba trafionych owiec w danym stadzie
	private int docelowa_l_owiec[];
	protected int rozdzieloneStada[];
	...
	protected JSONObject zamienNaJSON()
	{
		...
		try
		{
			...
			for (int i=0; i<5; i++)
			{
				wynik.put("docelowa_l_owiec"+String.valueOf(i),docelowa_l_owiec[i]);
				wynik.put("rozdzieloneStada"+String.valueOf(i),rozdzieloneStada[i]);
			}
	...
	protected void wczytajZJSON(JSONObject dane)
	{
		try
		{
			...
			for (int i=0; i<5; i++)
			{
			docelowa_l_owiec[i]=dane.getInt("docelowa_l_owiec"+String.valueOf(i));
			rozdzieloneStada[i]=dane.getInt("rozdzieloneStada"+String.valueOf(i));
			}
	...
	private void wejscie()
	{
		...
		docelowa_l_owiec=new int[5];
		rozdzieloneStada=new int[5];
		for (int i=0; i<5; i++) 
		{
			docelowa_l_owiec[i]=i;
			rozdzieloneStada[i]=0;
		}
		/// dzieki niej poinformuję Compa, kiedy stado zostanie zatopione już całkiem, komputer ustawi poprzednia_trafiona na true i zrozumie, ze nie ma juz co szukac kolejnych owiec, trzeba strzelac gdzie indziej
			...
			*/
			...
			switch (numer_stada) {
				...
				case 3: {
					...
					plansza.Kratka[x - 1][y - 1] = 6;
					savedSheeps[x-1][y-1]=1;
					int x1=x-1, y1=y-1, licznik=0;
					//sprawdzam czy stado zostalo rozdzielone
					if ((savedSheeps[x1+1][y1]==0 && startingBoard[x1+1][y1]==3) || savedSheeps[x1+1][y1]==2) licznik++;
					if ((savedSheeps[x1-1][y1]==0 && startingBoard[x1-1][y1]==3) || savedSheeps[x1-1][y1]==2) licznik++;
					if ((savedSheeps[x1][y1+1]==0 && startingBoard[x1][y1+1]==3) || savedSheeps[x1][y1+1]==2) licznik++;
					if ((savedSheeps[x1][y1-1]==0 && startingBoard[x1][y1-1]==3) || savedSheeps[x1][y1-1]==2) licznik++;
					Log.i("licznik", String.valueOf(licznik));
					if (licznik==2) rozdzieloneStada[3]=1;
					break;
				}
				case 4: {
					Fourth.trafienie();
					...
					int x1=x-1, y1=y-1, licznik=0;
					//sprawdzam czy stado zostalo rozdzielone
					if ((savedSheeps[x1+1][y1]==0 && startingBoard[x1+1][y1]==4) || savedSheeps[x1+1][y1]==2) licznik++;
					if ((savedSheeps[x1-1][y1]==0 && startingBoard[x1-1][y1]==4) || savedSheeps[x1-1][y1]==2) licznik++;
					if ((savedSheeps[x1][y1+1]==0 && startingBoard[x1][y1+1]==4) || savedSheeps[x1][y1+1]==2) licznik++;
					if ((savedSheeps[x1][y1-1]==0 && startingBoard[x1][y1-1]==4) || savedSheeps[x1][y1-1]==2) licznik++;
					if (licznik==3) rozdzieloneStada[4]=1;
					if (licznik==2)
					{
						int sprawdzenie=0;
						if ( ((savedSheeps[x1+1][y1]==0 && startingBoard[x1+1][y1]==4) || savedSheeps[x1+1][y1]==2) 
							&& ((savedSheeps[x1][y1+1]==0 && startingBoard[x1][y1+1]==4) || savedSheeps[x1][y1+1]==2)
							&& ((savedSheeps[x1+1][y1+1]==0 && startingBoard[x1+1][y1+1]==4) || savedSheeps[x1+1][y1+1]==2) ) sprawdzenie=1;
						...
						if (sprawdzenie==0) rozdzieloneStada[4]=1;
					}
					break;
				}
	...
	private void algorytm_wilka5454()
	{
			...
			if (rozdzieloneStada[docelowa_l_owiec[0]]==1)
			{
				//Jezeli atakowane stado jest rozdzielone to losujemy nowe pole.
				l_owiec=0; a1=1; a2=1; b1=1; b2=1; c1=1; c2=1; d1=1; d2=1; ile_sprawdzonych=0;
				docelowa_l_owiec[0]=0;
				poprzednia_trafiona=true;
			}
			//int x,y;
			if(poprzednia_trafiona)///jesli poprzednie stado jest juz calkiem odkryte lub rozdzielone, szukam nowego punktu zaczepienia
			{
				do
				{
					liczba1=rand.nextInt(s-2)+1;
					liczba2=rand.nextInt(s-2)+1;

				}
				while(plansza.Kratka[liczba1][liczba2]==5 || plansza.Kratka[liczba1][liczba2]==6);///dopoki wybierane jest pole, o ktore juz kiedys pytalam, bo Comp musi pamietac, ktore pola juz sprawdzal i nie pytac o nie wiecej niz raz
				if(plansza.Kratka[liczba1][liczba2]!=0) ///Gdy trafię, to przestawiam pop.._traf.. na false, bo odkryłam nowe stado, ktore dopiero będę odkrywala
				//tu juz wiadomo, ze wartosci od 0 do 4 wlacznie, 5 i 6 wykluczyl while
				{
					poprzednia_trafiona=false;
					docelowa_l_owiec[0]=plansza.Kratka[liczba1][liczba2];///poki jeszcze nie zmienilam wartosci na 5 lub 6 -> zapamietuje, zeby poinformowac komp., gdy juz bedzie stado odkryte
				}
			...
			if (numer_stada>0 && numer_stada<5)
				savedSheeps[liczba1][liczba2]=2;
			else
				savedSheeps[liczba1][liczba2]=4;
			...
			if(l_owiec==docelowa_l_owiec[docelowa_l_owiec[0]]) ///moge skorzystac z porownania do Kratki, bo
